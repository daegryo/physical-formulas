...
class El1(QMainWindow):
    def __init__(self):
       ...
       # self.theoryButton.clicked.connect(self.theory)
        ...
        self.result = self.cur.execute(f"""SELECT * FROM el1
                                                       WHERE number= '{11}'""").fetchall()
        st = '<br>'
        note = []
        ...
        image_path = f"images/practise/el/{self.result[0][4]}"
    ...
    def sh(self):
        ...
        if self.parameter == "Задание 11":
            self.num = 11
        elif self.parameter == "Задание 12":
            self.num = 12
        elif self.parameter == "Задание 13":
            self.num = 13
        elif self.parameter == "Задание 14":
            self.num = 14
        elif self.parameter == "Задание 15":
            self.num = 15
        elif self.parameter == "Задание 18":
            self.num = 18
        elif self.parameter == "Задание 21":
            self.num = 21
        elif self.parameter == "Задание 23":
            self.num = 23
        elif self.parameter == "Задание 25":
            self.num = 25
        ...
        for i in range(len(self.result[self.count][1])):
            st += self.result[self.count][1][i]
            count += 1
            if self.num != 21 and self.num!=23 and self.num != 18 and self.num != 25 and self.num != 14 and self.num != 15:
                if count == 46:
                    st += '<br>'  # Исправлено: только <br> без /
                    note.append(st)
                    count = 0
    ...
    def next(self):
        ...
        if self.count < self.counter and self.flag:
            ...
            if self.count + 1 <= len(self.result):
                self.numLabel.setText(f"{self.count + 1}/{len(self.result)}")
            image_path = f"images/practise/el/{self.result[self.count][4]}"
        ...
        if str(right_answ) == self.answerEdit.text():
            ...
            self.answerEdit.setStyleSheet(
                """background-color: rgba(255, 255, 255);
                    ...
                    border-radius: 3px;
                    """)
        if self.num == 21 or self.num == 23 or self.num == 25:
            ...
            self.decisionLabell.setStyleSheet(
                ...
                border-width:3px;
                border-style: solid;
                border-color: rgb(37, 171, 198);
                    """)

            image_path = f"images/practise/el/decision/{self.result[self.count][5]}"
