...
class Mkt(QMainWindow):
    def __init__(self):
        super().__init__()
        uic.loadUi('mkt.ui', self)
        ...
        self.result = self.cur.execute(f"""SELECT * FROM mkt
        ...
        with open(f"theory/mkt/1.txt", 'r', encoding='utf-8') as file:
            text = file.read()
            self.textLabel.setText(text)
            self.textLabel.setStyleSheet(
                """font: 13pt "Palatino Linotype";""")
            self.textLabel.setTextFormat(Qt.RichText)
        self.pixmap_formulas = QPixmap(f"images/theory/mkt/1/0.png")
    ...
    def speak_current_text_streaming(self):
            ...
            with open(f"theory/speak/mkt/{self.result[2]}.txt", 'r', encoding='utf-8') as file:
                text = file.read()
    ...
    def sh(self):
        ...
        self.result = self.cur.execute(f"""SELECT * FROM mkt
                                        WHERE name = '{self.parameter}'""").fetchone()
        if self.parameter == "Молекулярная физика":
            self.counter = 6
        elif self.parameter == "Термодинамика":
            self.counter = 10




        with open(f"theory/mkt/{self.result[1]}", 'r', encoding='utf-8') as file:
            text = file.read()
            self.textLabel.setText(text)
            self.textLabel.setStyleSheet(
                """font: 13pt "Palatino Linotype";""")
        self.pixmap_formulas = QPixmap(f"images/theory/mkt/{self.result[2]}/{self.count}.png")
        self.photoLabel.setPixmap(self.pixmap_formulas)
